# This script will read TiAlN GADDS out files.
...
name_of_outfileinput = 'outFiles//GADDS//decided//TiAlN560.out'
...
# INPUT 
expected_peaks = [37.4 , 43.5 , 63.29, 75.9 , 79.8, 95.6]
# INPUT 


#This will fit on the scatter and return the lmfit result
result = functions.fit_experimental_data(spec.x,spec.y,expected_peaks,deg_of_bck_poly=1)
...
a1.plot(two_theta, residual, lw = 1.1)
a0.set_ylim([0,120])
a1.set_ylim([-10,10])
a0.set_xlim([33,105])
a1.set_xlim([33,105])
...
#a0.set_title('{} data points \n GADDS '.format(len(two_theta)),fontsize = 14)
a1.set_xticklabels([])
...
for i in range(6):
    _,deconv_popt = functions.get_deconvoluted_profile(fwhm,peak_two_thetas,amplitude,fraction,d_hkl,
                                                       al_fwhm,al_fraction, i, plot = True)
...
#Deconvolution - Brute  : This takes a long time!
##after_deconv   = []
##
...
##print('Coefficients after deconvolution')    
##print(after_deconv)
