# This script will read TiAlN D5000 out files.
...
######lorentz = (1 + np.multiply(np.cos(two_theta*np.pi/180),np.cos(two_theta*np.pi/180))/np.multiply(np.sin(two_theta*np.pi/360),np.sin(two_theta*np.pi/360)))
######intensity = np.divide(np.array(intensity),lorentz)
...
a1.scatter(two_theta, residual, s = 8)
a0.set_ylim([0,120])
a1.set_ylim([-20,20])
a0.set_xlim([35,105])
a1.set_xlim([35,105])
...
a0.set_title('{} data points \n D5000 '.format(len(two_theta)),fontsize = 14)
a1.set_xticklabels([])
...
#Deconvolution
al_fwhm     = [functions.caglioti(i,*uvw_D5000) for i in peak_two_thetas]
